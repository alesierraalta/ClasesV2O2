...
# Configurar para manejar las barras diagonales finales (trailing slashes)
...
class HorarioClase(db.Model):
    ...
    def hora_fin_str(self):
        ...
        try:
            logging.debug(f"hora_fin_str para clase {self.id} '{self.nombre}' - hora_inicio={self.hora_inicio}, duracion={self.duracion}")
            ...
            minutos_totales = hora_inicio.hour * 60 + hora_inicio.minute + self.duracion
            logging.debug(f"minutos_totales = {minutos_totales}")
            
            horas, minutos = divmod(minutos_totales, 60)
            logging.debug(f"horas antes modulo = {horas}, minutos = {minutos}")
            
            # Asegurar que las horas estén en formato 24h (0-23)
            horas = horas % 24
            logging.debug(f"horas después modulo = {horas}")
            
            resultado = f"{horas:02d}:{minutos:02d}"
            logging.debug(f"hora_fin_str resultado final = {resultado}")
            return resultado
        except Exception as e:
            logging.error(f"Error en hora_fin_str: {str(e)}")
            return "00:00"
...
def informe_mensual():
    ...
    if request.method == 'GET' and 'mes' in request.args and 'anio' in request.args:
        ...
        # Consultar clases realizadas en el rango de fechas usando la base de datos
        # Asegurarse de que la conexión está limpia
        db.session.commit()
        db.session.close()
        db.session = db.create_scoped_session()
        
        # Consulta SQL directa para obtener los datos de clase
        sql = """
        SELECT 
        ...
        debug_log(f"Ejecutando SQL: {sql}")
        
        try:
            conn = db.engine.connect()
            result = conn.execute(sql, {'fecha_inicio': primer_dia, 'fecha_fin': ultimo_dia})
            ...
            for row in result:
                try:
                    ...
                    # Procesar hora_inicio para mostrarla correctamente
                    hora_inicio_raw = row.hora_inicio
                    debug_log(f"Clase {row.id}: hora_inicio_raw = {hora_inicio_raw}, tipo = {type(hora_inicio_raw)}")
                    
                    # Convertir hora_inicio a objeto time
                    try:
                        if isinstance(hora_inicio_raw, str):
                            if '.' in hora_inicio_raw:  # Tiene microsegundos
                                hora_inicio_str = hora_inicio_raw.split('.')[0]
                            else:
                                hora_inicio_str = hora_inicio_raw
                            
                            # Convertir según el formato
                            if hora_inicio_str.count(':') == 1:  # HH:MM
                        ...
                        debug_log(f"Clase {row.id}: hora_inicio_obj = {hora_inicio_obj}")
                        
                        # Calcular hora_fin utilizando hora_inicio_obj y duración
                        duracion_mins = row.duracion if row.duracion is not None else 60
                        ...
                        horas_fin = horas_fin % 24  # Asegurar formato 24h
                        
                        hora_fin_str = f"{horas_fin:02d}:{minutos_fin:02d}"
                        debug_log(f"Clase {row.id}: hora_fin_str calculada = {hora_fin_str}")
                    except Exception as e:
                        debug_log(f"Error al calcular hora_fin para clase {row.id}: {str(e)}")
                        # Try to get hora_fin_str from HorarioClase model instance if possible
                        horario = HorarioClase.query.get(row.horario_id)
                        if horario:
                            hora_fin_str = horario.hora_fin_str()
                            debug_log(f"Obtenida hora_fin_str desde el modelo: {hora_fin_str}")
                        else:
                            hora_fin_str = "??:??"
                            debug_log(f"No se pudo obtener hora_fin_str, usando valor por defecto")
                        # Set a default hora_inicio_obj for further processing
                        hora_inicio_obj = time(0, 0)
                    
                    # Procesar hora_llegada_profesor
                    hora_llegada = row.hora_llegada_profesor
                    if hora_llegada and isinstance(hora_llegada, str):
                        if '.' in hora_llegada:  # Tiene microsegundos
                            hora_llegada = hora_llegada.split('.')[0]
                        
                        try:
                            if hora_llegada.count(':') == 1:  # HH:MM
                                hora_llegada = datetime.strptime(hora_llegada, '%H:%M').time()
                            else:  # HH:MM:SS
                                hora_llegada = datetime.strptime(hora_llegada, '%H:%M:%S').time()
                        except Exception as e:
                            debug_log(f"Error al convertir hora_llegada: {str(e)}")
                            hora_llegada = None
                    
                    # Calcular estado de puntualidad
                    if hora_llegada:
                        minutos_antes = (hora_inicio_obj.hour * 60 + hora_inicio_obj.minute) - (hora_llegada.hour * 60 + hora_llegada.minute)
                        
                        if minutos_antes >= 15:
                            estado_puntualidad = "Muy puntual"
                        elif minutos_antes >= 5:
                            estado_puntualidad = "Puntual"
                        elif minutos_antes >= -5:
                            estado_puntualidad = "A tiempo"
                        elif minutos_antes >= -15:
                            estado_puntualidad = "Leve retraso"
                        else:
                            estado_puntualidad = "Retraso significativo"
                    else:
                        estado_puntualidad = "No registrada"
                    
                    # Crear diccionario para la clase con todos los datos procesados
                    clase = {
                        'id': row.id,
                        'fecha': fecha,
                    ...
                    debug_log(f"Clase procesada ID {row.id}: {row.nombre} - {hora_inicio_obj.strftime('%H:%M')} - {hora_fin_str}")
                    
                except Exception as e:
                    debug_log(f"Error al procesar clase ID {row.id}: {str(e)}")
                    continue
            
            # Procesar datos para el resumen
            total_clases = len(clases_realizadas)
            total_alumnos = sum(clase['cantidad_alumnos'] for clase in clases_realizadas if clase['cantidad_alumnos'])
            total_pago = sum(clase['profesor']['tarifa_por_clase'] for clase in clases_realizadas)
            
            # Clases no registradas
            clases_no_registradas = []
            
            # Conteo de clases no registradas por tipo
            conteo_no_registradas = defaultdict(int)
            for clase in clases_no_registradas:
                tipo_clase = clase.get('tipo_clase', 'OTRO')
                conteo_no_registradas[tipo_clase] += 1
            
            # Resúmenes por profesor y tipo
            resumen_profesores = {}
            resumen_tipos = {}
            
            for clase in clases_realizadas:
                # Resumir por profesor
                profesor_id = clase['profesor_id']
                if profesor_id not in resumen_profesores:
                    resumen_profesores[profesor_id] = {
                        'id': profesor_id,
                ...
                resumen_profesores[profesor_id]['total_clases'] += 1
                resumen_profesores[profesor_id]['total_alumnos'] += clase['cantidad_alumnos'] or 0
                
                if clase['puntualidad'] in ["Leve retraso", "Retraso significativo"]:
                    resumen_profesores[profesor_id]['total_tardanzas'] += 1
                
                resumen_profesores[profesor_id]['pago_total'] += clase['profesor']['tarifa_por_clase']
                
                # Resumir por tipo de clase
                tipo_clase = clase['horario']['tipo_clase']
                if tipo_clase not in resumen_tipos:
                    resumen_tipos[tipo_clase] = {
                        'tipo': tipo_clase,
                        'total_clases': 0,
                        'total_alumnos': 0
                    }
                
                resumen_tipos[tipo_clase]['total_clases'] += 1
                resumen_tipos[tipo_clase]['total_alumnos'] += clase['cantidad_alumnos'] or 0
            
            # Convertir resúmenes a listas para la plantilla
            resumen_profesores_lista = list(resumen_profesores.values())
            resumen_profesores_lista.sort(key=lambda x: x['total_clases'], reverse=True)
            
            resumen_tipos_lista = list(resumen_tipos.values())
            resumen_tipos_lista.sort(key=lambda x: x['total_clases'], reverse=True)
            ...
            return render_template('informes/mensual_resultado.html', 
                                ...
                                clases_realizadas=clases_realizadas,
                                clases_no_registradas=clases_no_registradas,
                                conteo_no_registradas=dict(conteo_no_registradas),
                                resumen_profesores=resumen_profesores_lista,
                                resumen_tipos=resumen_tipos_lista,
                                total_clases=total_clases,
                                total_alumnos=total_alumnos,
                                total_pago=total_pago)
        
        except Exception as e:
            debug_log(f"Error general en informe_mensual: {str(e)}")
            raise
    
    # Para peticiones GET sin parámetros (mostrar formulario)
    hoy = datetime.now()
                ...
                for fila_num, row in df.iterrows():
                    try:
                        ...
                        if not horario:
                            ...
                            # Ensure time value is correct by logging it
                            logging.debug(f"Creando horario: {clase_nombre} con hora_inicio={hora}")
                            db.session.add(horario)
                            db.session.commit()
                            logging.debug(f"Horario creado con ID={horario.id}, hora_inicio={horario.hora_inicio}")
...
def exportar_db():
    ...
    export_filename = f'gimnasio_backup_{timestamp}.db'
    
    # Asegurarse de que haya conexión a la base de datos
    try:
        conn = sqlite3.connect(db_file)
        conn.close()
    except Exception as e:
        flash(f'Error al acceder a la base de datos: {str(e)}', 'danger')
...
def exportar_db_completo():
    """Exportar un archivo ZIP con la base de datos y los archivos de audio."""
    import zipfile
    import io
    
    # Configurar logging para depuración
    logging.basicConfig(level=logging.DEBUG)
    logger = logging.getLogger('backup_debug')
    
    # Crear un archivo ZIP en memoria
    memory_file = io.BytesIO()
    
    try:
        logger.debug("Iniciando exportación de backup completo")
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
            # Añadir la base de datos
            db_file = 'gimnasio.db'
            if os.path.exists(db_file):
                zipf.write(db_file, os.path.basename(db_file))
                logger.debug(f"Base de datos añadida al ZIP: {db_file}")
            else:
                logger.warning(f"Archivo de base de datos no encontrado: {db_file}")
            
            # Añadir los archivos de audio
            audio_folder = os.path.join('static', 'audio')
            if os.path.exists(audio_folder):
                logger.debug(f"Carpeta de audio encontrada: {audio_folder}")
                audio_files_count = 0
                audio_files_size = 0
                
                for root, dirs, files in os.walk(audio_folder):
                    logger.debug(f"Escaneando directorio: {root}, contiene {len(files)} archivos")
                    for file in files:
                        file_path = os.path.join(root, file)
                        # Almacenar con ruta relativa en el ZIP
                        arcname = os.path.relpath(file_path)
                        
                        # Verificar tamaño del archivo
                        file_size = os.path.getsize(file_path)
                        audio_files_size += file_size
                        
                        zipf.write(file_path, arcname)
                        audio_files_count += 1
                        logger.debug(f"Archivo añadido al ZIP: {arcname}, tamaño: {file_size} bytes")
                
                logger.info(f"Total de archivos de audio añadidos: {audio_files_count}, tamaño total: {audio_files_size} bytes")
            else:
                logger.warning(f"Carpeta de audio no encontrada: {audio_folder}")
        
        # Obtener el tamaño final del ZIP
        zip_size = memory_file.tell()
        logger.info(f"Archivo ZIP creado correctamente, tamaño: {zip_size} bytes")
    ...
    return send_file(
        memory_file,
        as_attachment=True,
        download_name=f'gimnasio_backup_completo_{timestamp}.zip',
...
def importar_db():
    ...
    try:
        ...
        # Verificar si es un archivo de base de datos
        ...
        # Crear una copia de seguridad de la BD actual
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        backup_filename = f'gimnasio_pre_import_{timestamp}.db'
        ...
        if os.path.exists(db_path):
            ...
            flash(f'Se creó una copia de seguridad de la base de datos actual: {backup_filename}', 'info')
        
        # Guardar el archivo nuevo
        db_file.save(db_path)
        
        flash('Base de datos importada correctamente. Reinicie la aplicación para aplicar los cambios.', 'success')
        return redirect(url_for('configuracion_exportar'))
...
def importar_db_completo():
    """Importar un archivo ZIP con la base de datos y los archivos de audio."""
    try:
        # Verificar si se adjuntó un archivo
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Verificar si contiene el archivo de base de datos
            db_temp_path = os.path.join(temp_dir, 'gimnasio.db')
            if not os.path.exists(db_temp_path):
                flash('El archivo ZIP no contiene una base de datos válida (gimnasio.db)', 'danger')
                return redirect(url_for('configuracion_exportar'))
            ...
            backup_db_path = os.path.join(backup_dir, f'gimnasio_pre_import_{timestamp}.db')
            if os.path.exists(db_path):
                shutil.copy2(db_path, backup_db_path)
            
            # Backup de los archivos de audio
            audio_dir = os.path.join('static', 'audio')
            backup_audio_dir = os.path.join(backup_dir, f'audio_backup_{timestamp}')
            if os.path.exists(audio_dir) and os.listdir(audio_dir):
                shutil.copytree(audio_dir, backup_audio_dir)
                flash(f'Se creó una copia de seguridad de los archivos de audio en: {backup_audio_dir}', 'info')
            
            # Copiar la nueva base de datos
            shutil.copy2(db_temp_path, db_path)
            
            # Copiar los archivos de audio del backup si existen
            temp_audio_dir = os.path.join(temp_dir, 'static', 'audio')
            if os.path.exists(temp_audio_dir):
                # Asegurarse de que el directorio de destino existe
                os.makedirs(audio_dir, exist_ok=True)
                
                # Copiar cada archivo de audio
                for root, dirs, files in os.walk(temp_audio_dir):
                    for file in files:
                        src_path = os.path.join(root, file)
                        # Calcular la ruta relativa
                        rel_path = os.path.relpath(src_path, temp_audio_dir)
                        dst_path = os.path.join(audio_dir, rel_path)
        ...
        flash('Backup completo importado correctamente. Reinicie la aplicación para aplicar los cambios.', 'success')
        return redirect(url_for('configuracion_exportar'))
        
    except Exception as e:
        app.logger.error(f"Error importando el backup completo: {str(e)}")
...
logging.basicConfig(
    filename=os.path.join(log_dir, 'hora_debug.log'),
    level=logging.DEBUG,
